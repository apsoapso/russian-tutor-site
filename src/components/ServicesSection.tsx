...
const ServiceCard: React.FC<ServiceCardProps> = ({ icon, title, description, features, cta }) => {
  return (
    <Card className="border-gold/20 hover:border-gold/50 transition-all duration-300 h-full flex flex-col bg-white/80">
      <CardHeader>
        <div className="w-12 h-12 rounded-full bg-burgundy/10 flex items-center justify-center mb-4">
          <Icon name={icon} className="h-6 w-6 text-burgundy" />
        </div>
        ...
        <ul className="space-y-2">
          {features.map((feature, index) => (
            <li key={index} className="flex items-start">
              <Icon name="Check" className="h-5 w-5 text-gold mr-2 mt-0.5 flex-shrink-0" />
      ...
      <CardFooter>
        <Button className="w-full bg-burgundy text-cream hover:bg-burgundy/90">{cta}</Button>
      </CardFooter>
    </Card>
  );
};
  ...
  const services = [
    {
      ...
      description: "Целенаправленная подготовка к успешной сдаче ЕГЭ по русскому языку и литературе",
      features: [
    ...
    },
    {
      ...
      description: "Эффективная подготовка к успешной сдаче ОГЭ по русскому языку и литературе",
      features: [
    ...
    },
    {
      ...
      description: "Углубленное изучение русского языка и литературы для увлеченных предметом",
      features: [
    ...
    <section id="services" className="section bg-cream py-20">
      <div className="container">
        ...
        <p className="section-subtitle">
          Предлагаю различные форматы занятий, адаптированные под ваши цели и уровень подготовки
        </p>
        ...
        <div className="mt-16 text-center">
          <p className="mb-6 text-lg">Не нашли подходящий формат? Свяжитесь со мной для обсуждения индивидуальной программы</p>
